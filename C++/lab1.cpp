...
int fun3_b(int x,int* array)//分解1个数的所有质因数(包括重复数字),并存入对应的数组中
{
   // cout << x << endl;
    int cnt = 0;
    while (x > 1)
...
int fun3_a(int m, int n) {//算法3 分解质因数
    int ans = 1;
    int QAQ = fun3_b(m, is_prime1);//存储其质因数个数
    int QWQ = fun3_b(n, is_prime2);
    int a = 0, b = 0;
    while (a < QAQ && b < QWQ)//一一比较,找出所有的公共质因数(包含重复)
    {
        ...
        else if (is_prime1[a] > is_prime2[b])//因为公共质因数数组是按照从小到大排序的,一次如果不同,则往后查找
        {
            b++;
        }
        else
