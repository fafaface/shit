...
IMPLEMENT_DYNAMIC(CDlgIntensity, CDialogEx)

CDlgIntensity::CDlgIntensity(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_DLG_Intensity, pParent)
{
...
BEGIN_MESSAGE_MAP(CDlgIntensity, CDialogEx)
	ON_EN_KILLFOCUS(IDC_EDIT_LowGray, &CDlgIntensity::OnEnKillfocusEditLowgray)
	ON_EN_KILLFOCUS(IDC_EDIT_UpGray, &CDlgIntensity::OnEnKillfocusEditUpgray)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_WM_PAINT()
	ON_BN_CLICKED(IDCANCEL, &CDlgIntensity::OnBnClickedCancel)
END_MESSAGE_MAP()


// CDlgIntensity 消息处理程序


void CDlgIntensity::OnEnKillfocusEditLowgray()
{
	// TODO: 在此添加控件通知处理程序代码
	// 保存变动（控件中数值保存到成员变量中）
	UpdateData(TRUE);
...
void CDlgIntensity::OnEnKillfocusEditUpgray()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CDlgIntensity::OnBnClickedCancel()
{
	// TODO: 在此添加控件通知处理程序代码
	CDialogEx::OnCancel();
}
