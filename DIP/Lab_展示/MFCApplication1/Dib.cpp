					...
					CPalette* pPal)
{
	...
	//GlobalLock((HGLOBAL)返回指向该内存块的度指针并锁定该内存块(只能本线程读写而防止其他线知程读写)；
...
HGLOBAL CDib::LoadFile(CFile& file)
{
	...
	int width = lpbmihrd->biWidth;
	int height = lpbmihrd->biHeight;



	if (((BITMAPINFOHEADER*)lpdib)->biBitCount == 8)
	{
		int dataSizePerLine = (width * 8 / 8 + 3) / 4 * 4;
		lpdib2 = new unsigned char[40 + 1024 + dataSizePerLine*height];
		memcpy(lpdib2, lpdib, 40 + 1024 + dataSizePerLine*height);
	}

	if (((BITMAPINFOHEADER*)lpdib)->biBitCount == 24)
	{
		int dataSizePerLine = (width * 24 / 8 + 3) / 4 * 4;
		lpdib2 = new unsigned char[40 + dataSizePerLine*height];
		memcpy(lpdib2, lpdib, 40 + dataSizePerLine*height);
	}
